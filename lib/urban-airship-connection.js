    ...
    request = https.request(requestOptions, function (response) {
        ...
        response.on('end', function() {
            ...
            if (response.statusCode === connectionOptions.expectedStatusCode) {
                ...
                var errorDetails = rawResponseData;
                if ( parsedResponseData ) {
                    errorDetails = parsedResponseData.error;
                    if ( typeof parsedResponseData.details != 'undefined' && parsedResponseData.details ) {
                        errorDetails += " ("+parsedResponseData.details.error+")";
                    }
                }
                printError(response.statusCode, parsedResponseData ? parsedResponseData.error : rawResponseData);
                callback(new Error('Unexpected answer: '+errorDetails));
            }
        });
